...
function makeCard(id) {
  var newCard = { number: id,
    ...
    cardID: function (rank, suit) { // --> 0..51
        if ((makeCard.inRange(rank, 1, 13)) && (makeCard.inRange(suit, 1, 4))) {
    ...
    precedes: function (cardA, cardB) { //-->false,true
       if ((makeCard.inRange(cardA, 0, 51)) && (makeCard.inRange(cardB, 0, 51))) {
    ...
    nextInSuit: function (cardA) {//--> 0..51
        if (makeCard.inRange(cardA, 0, 51)) {
            if (cardA < 48) {
                return (cardA + 4);
            } else {
                return (cardA + 4 - 52);
    ...
    prevInSuit: function (cardB) {//--> 0..51
        if (makeCard.inRange(cardB, 0, 51)) {
            if (cardB > 3) {
                return (cardB - 4);
            } else {
                return (cardB - 4 + 52);
